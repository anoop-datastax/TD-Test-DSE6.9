...
public class CassandraAsyncWriter {
    ...
    private static final long TOTAL_ROWS = 20_000_000; // 20 million rows
    private static final long ASYNC_WRITE_BATCH_SIZE = 1000; // Process 1000 rows at a time
    ...
    public static void main(String[] args) throws ExecutionException, InterruptedException {
        try (CqlSession session = CqlSession.builder()
            ...
            AtomicLong totalInsertedRows = new AtomicLong(0);

            CompletableFuture<Void> future = CompletableFuture.completedFuture(null);

            for (long i = 0; i < TOTAL_ROWS; i += ASYNC_WRITE_BATCH_SIZE) {
                // Create a batch of 1000 async writes
                for (long j = i; j < i + ASYNC_WRITE_BATCH_SIZE && j < TOTAL_ROWS; j++) {
                    boolean meetsCondition = j < TOTAL_ROWS / 2;
                    ...
                    // Chain the async write
                    future = future.thenCompose(v -> session.executeAsync(boundStatement).thenAcceptAsync(rs -> {
                        // Increment total rows inserted after each successful write
                        long insertedRows = totalInsertedRows.incrementAndGet();
                ...
                // Wait for all async operations for this batch to complete before starting the next batch
                future.get();
            }
